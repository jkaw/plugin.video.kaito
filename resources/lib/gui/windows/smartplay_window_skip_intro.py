...
class SmartPlayWindowSkipIntro(BaseWindow):
    """
    ...
    def __init__(self, xml_file, xml_location, item_information=None):
        try:
            super(SmartPlayWindowSkipIntro, self).__init__(
    ...
    def onInit(self):
        ...
        super(SmartPlayWindowSkipIntro, self).onInit()
    ...
    def close(self):
        ...
        """
        self.closed = True
        super(SmartPlayWindowSkipIntro, self).close()
    ...
    def handle_action(self, action, control_id=None):
        if action == 7:
            ...
            if control_id == 3003:
                ...
                self.close()
            if control_id == 3004:
                self.player.seekTime(self.player.getTime() + int(g.get_setting('skipintro.time')))
                self.close()
