...
def premiumize_transfer_cleanup():
    """
    Cleanup transfers created by Kaito at Premiumize
    :return: None
    :rtype: NOne
    """
    ...
    if fair_usage < threshold:
        ...
        return
    kaito_transfers = premiumize_transfers.get_premiumize_transfers()
    if kaito_transfers is None:
        g.log("Failed to cleanup transfers, API error", "error")
        return
    if len(kaito_transfers) == 0:
        g.log("No Premiumize transfers have been created")
        return
    g.log("Premiumize Fair Usage is above threshold, cleaning up Kaito transfers")
    for i in kaito_transfers:
