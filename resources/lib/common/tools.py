...
def get_episode_information(anilist_id, episode, season=None):
    """
    Ease of use tool to retrieve items meta from TraktSyncDatabase based on action arguments
    :param anilist_id: anilist id of show
    :param episode: episode number of show
    :param season: season number of show
    :type anilist_id: int
    :type episode: int
    :type season: int
    :return: Metadata for item
    ...
    if anilist_id is None:
        return None
    from resources.lib.database.anilist_sync import shows
    return shows.AnilistSyncDatabase().get_episode(anilist_id, episode, season)
