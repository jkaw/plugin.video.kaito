...
class TRAKTAPI(object):
    ...
    def _process_trakt_episode_view(self, anilist_id, show_meta, season, poster, fanart, eps_watched, url, data,
                                    base_plugin_url):
        from datetime import datetime, timedelta
        update_time = (datetime.today() + timedelta(days=5)).strftime('%Y-%m-%d')
        result = self._json_request(url)
        mapfunc = partial(self._parse_trakt_episode_view, show_id=anilist_id, show_meta=show_meta, season=season,
    ...
    def get_trakt_id_backup(self, item_information):
        ...
        if len(result) == 1:
            ...
            if not show:
                ...
                show = next((i['show'] for i in result if i['show'].get('aired_episodes') == episode_count),
                            result[0]['show'])
    ...
    def get_anime(self, anilist_id, db_correction):
        ...
        meta_ids = tools.get_item_information(anilist_id)
    ...
    def get_trakt_episodes(self, show_id, season):
        ...
        data = ''
        return self._process_trakt_episode_view(show_id, show_meta, season, poster, fanart, eps_watched, url, data,
                                                "animes_page/%s/%%d" % show_id)
