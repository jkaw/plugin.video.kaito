...
def get(function, duration, *args, **kwargs):
    ...
    try:
        ...
        if 'animepahe_reload' in kwargs:
            reload = kwargs['kaito_reload']
            kwargs.pop('kaito_reload')

        if 'animepahe_sources' in kwargs:
            sources = True
            kwargs.pop('kaito_sources')
...
def cache_get(key):
    try:
        ...
        tools.try_release_lock(migrate_db_lock)
    ...
    try:
        ...
        for t in [cache_table, 'rel_list', 'rel_lib']:
            try:
                ...
                pass
        tools.showDialog.notification('{}: {}'.format(g.ADDON_NAME, g.lang(30200)), g.lang(30201), time=5000)
    except:
        pass
    finally:
        tools.try_release_lock(migrate_db_lock)
...
def torrent_cache_clear():
    confirmation = tools.yesno_dialog(g.ADDON_NAME, "Are you sure you wish to clear the cache?")
    if not confirmation:
        return
    try:
        migrate_db_lock.acquire()
    ...
    tools.showDialog.notification('{}: {}'.format(g.ADDON_NAME, g.lang(30200)), g.lang(30202), time=5000)

def clearSearchHistory():
    try:
        migrate_db_lock.acquire()
        confirmation = tools.yesno_dialog(g.ADDON_NAME, "Clear search history?")
        if not confirmation:
            return
        cursor = _get_connection_cursor(g.SEARCH_HISTORY_DB_PATH)
    ...
    tools.showDialog.notification(g.ADDON_NAME, "Search history has been cleared", time=5000)
