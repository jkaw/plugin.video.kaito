...
class KaitoPlayer(xbmc.Player):
    ...
    def play_source(self, stream_link, item_information, resume_time=None):
        ...
        #    item = self.smart_module.get_play_menu_item(item_information)[0][1]
        #    item.setPath(stream_link)
        #else:
        item = self._create_list_item(stream_link)
    ...
    def _keep_alive(self):
        ...
        while self._is_file_playing() and not g.abort_requested():
            ...
            if self.mediatype != 'movie' and g.get_setting('smartplay.skipintrodialog') == 'true' and not self.skipped_intro:
                if int(self.current_time) < 240:
                    xbmc.executebuiltin(
                        'RunPlugin("plugin://plugin.video.kaito/?action=runPlayerSkipIntro")'
                    )
            ...
            if (self.watched_percentage >= self.playCountMinimumPercent) and not self.updated:
            ...
            if self.dialogs_enabled and not self.dialogs_triggered:
                if time_left <= self.playing_next_time:
                    xbmc.executebuiltin(
                        'RunPlugin("plugin://plugin.video.kaito/?action=runPlayerDialogs")'
                        )
    ...
    def onWatchedPercent(self):
        ...
        from ..modules.WatchlistIntegration import watchlist_update
        watchlist_update(self.item_information['anilist_id'], self.item_information['episode'])
        if self.item_information.get('season') and self.media_type != 'movie':
            self.shows_sync.mark_episode_watched(
                self.args['anilist_id'],
                self.season,
                self.args['episode']
...
class PlayerDialogs(xbmc.Player):
    ...
    def _show_skip_intro():
        ...
        try:
            window = SkipIntro(*('skip_intro.xml', g.ADDON_DATA_PATH))
            window.doModal()
        finally:
            del window
    ...
    def _get_next_item_item_information():
        ...
        next_info = {"art": {}, "info": {}}
        next_info['info']['episode'] = next_item.getVideoInfoTag().getEpisode()
        next_info['info']['season'] = next_item.getVideoInfoTag().getSeason()
        next_info['info']['tvshowtitle'] = next_item.getVideoInfoTag().getTVShowTitle()
        next_info['info']['rating'] = next_item.getVideoInfoTag().getRating()
        next_info['art']['thumb'] = next_item.getArt('thumb')
        next_info['info']['title'] = next_item.getLabel()
