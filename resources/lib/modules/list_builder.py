...
class ListBuilder(object):
    ...
    def season_list_builder(self, show_id, trakt_show_id, **params):
        """
    ...
    def episode_list_builder(self, anilist_id, trakt_show_id, trakt_season=None, **params):
        """
    ...
    def episode_alt_list_builder(self, anilist_id, trakt_season=None, **params):
        """
        Builds a menu list of a episodes for specified show's season
        :param trakt_show_id: Trakt ID of show
        :param trakt_season: season number
        ...
        return self._common_menu_builder(
            shows.AnilistSyncDatabase().get_alt_episode_list(
                anilist_id,
                trakt_season,
                minimum_episode=params.pop("minimum_episode", None),
                **params
    ...
    def show_list_builder(self, trakt_list, **params):
        ...
        self._common_menu_builder(
            shows.AnilistSyncDatabase().get_show_list(trakt_list, **params),
            g.CONTENT_SHOW,
            action,
            **params
        )
    ...
    #     self._common_menu_builder(
    #         movies.AnilistSyncDatabase().get_movie_list(trakt_list, **params),
    #         g.CONTENT_MOVIE,
    #         action,
    #         **params
    #     )
    ...
    def _common_menu_builder(
        ...
        if "no_paging" in params:
            ...
            no_paging = g.REQUEST_PARAMS.pop("no_paging", False)
    ...
    def _post_process(self, item, prepend_date=False, mixed_list=False):
        ...
        if (
            ...
            and item["info"]["episode_count"] == 1
        ):
            item.update({'is_movie': True})

        if item["info"]["mediatype"] == "episode":
            if not item.get("args"):
                item.update({"args": item["info"]["args"]})
