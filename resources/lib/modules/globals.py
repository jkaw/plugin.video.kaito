...
class GlobalVariables(object):
    ...
    def _init_paths(self):
        ...
        self.ANILIST_SYNC_DB_PATH = tools.translate_path(
        ...
        self.KODI_VIDEO_DB_PATH = tools.translate_path(
    ...
    def clear_kodi_bookmarks(self):
        with self.get_kodi_video_db_connection() as video_database:
            file_ids = [str(i["idFile"]) for i in video_database.fetchall(
                "SELECT * FROM files WHERE strFilename LIKE '%plugin.video.kaito%'")]
            if not file_ids:
                return
    ...
    def myanimelist_enabled(self):
        if self.get_setting('mal.token') != '' and self.get_bool_setting('mal.enabled'):
            return True
        else:
            return False

    def kitsu_enabled(self):
        if self.get_setting('kitsu.token') != '' and self.get_bool_setting('kitsu.enabled'):
            return True
        else:
            return False

    def anilist_enabled(self):
        if self.get_setting('anilist.token') != '' and self.get_bool_setting('anilist.enabled'):
            return True
        else:
            return False
    ...
    def add_directory_item(self, name, **params):
        ...
        #        from resources.lib.common import database
    ...
    def open_addon_settings(self, section_offset, setting_offset=None):
        """
        Open kaito settings at a particular section and setting
