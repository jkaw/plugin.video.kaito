...
class AnilistSyncDatabase(anilist_sync.AnilistSyncDatabase):
    """
    ...
    def extract_trakt_page(self, url, **params):
        ...
        :rtype: list
        """
        return super(AnilistSyncDatabase, self)._extract_trakt_page(
            url, "shows", **params
        )

    def extract_animelist_page(self, url, **params):
        """
        Extracts items from page
        :param url: URL endpoint to extract
        :type url: string
        ...
        :rtype: list
        """
        return super(AnilistSyncDatabase, self)._extract_animelist_page(
            url, "shows", **params
        )

    @guard_against_none()
    def _update_shows_statistics_from_show_id(self, trakt_show_id):
        self.update_shows_statistics([{"anilist_id": trakt_show_id}])
    ...
    def mark_episode_watched(self, show_id, season, number):
        ...
        play_count = self.fetchone(
            ...
            "where anilist_id=? and season=? and number=?",
    ...
    @guard_against_none()
    def mark_show_record(self, column, value, show_id):
        self.execute_sql(
            "UPDATE shows SET {}=? WHERE anilist_id=?".format(column), (value, show_id)
        )

    @guard_against_none()
    def _mark_show_record(self, value, show_id):
        self.execute_sql(
            "UPDATE shows SET {}=? WHERE anilist_id=?", (value, show_id)
        )

    @guard_against_none()
    def update_show_art(self, key, value, show_id):
        if value:
            art = self.get_show(show_id).get('art')
            art[key] = value
            self.execute_sql(
                "UPDATE shows SET art=? WHERE anilist_id=?", (art, show_id)
            )
        else:
            pass
    ...
    def _mark_episode_record(self, column, value, show_id, season, number):
        ...
        if datetime_column is None:
            ...
            raise ValueError
        self.execute_sql(
            "UPDATE episodes SET {}=?, {}=? WHERE anilist_id=? AND season=? AND "
    ...
    def get_show_list(self, trakt_list, **params):
        ...
        trakt_list = [i for i in trakt_list if i.get("anilist_id")]
        ...
        statement = """SELECT s.anilist_id, s.mal_id, s.simkl_id, s.trakt_id, s.tmdb_id, s.info, s.cast, s.art, s.args, 
        s.watched_episodes, s.unwatched_episodes, s.episode_indexer, s.episode_count, s.episode_offset, s.air_date FROM shows as s 
        WHERE s.anilist_id in ({}) """.format(",".join((g.UNICODE(i.get("anilist_id")) for i in trakt_list)))
    ...
    def get_season_list(self, anilist_id, trakt_show_id, trakt_id=None, **params):
        """
        ...
        self._try_update_seasons(anilist_id, trakt_show_id)
        g.log("Updated requested seasons", "debug")
        statement = """SELECT s.anilist_id, s.trakt_id, s.info, s.cast, s.art, s.args, s.watched_episodes, s.unwatched_episodes, 
        s.episode_count FROM seasons AS s WHERE """
        if trakt_id is not None:
            statement += "s.trakt_id == {}".format(trakt_id)
        else:
            statement += "s.anilist_id == {}".format(anilist_id)
        # # if params.pop("hide_unaired", self.hide_unaired):
        # #     statement += " AND Datetime(s.air_date) < Datetime('now')"
    ...
    def get_episode_list(
        ...
        **params
        ...
        :param anilist_id: Anilist ID of show
        :type anilist_id: int
        :param trakt_show_id: Trakt ID of show
        :type trakt_show_id: int
    ...
    def get_mixed_episode_list(self, trakt_items, **params):
        ...
        #     )
        # else:
        query = """SELECT e.trakt_id, e.anilist_id, e.info, e.cast, e.art, e.args, e.watched as play_count
        FROM episodes as e WHERE e.trakt_id in ({})""".format(
            in_predicate
        )
    ...
    @guard_against_none()
    def _get_single_trakt_show_meta(self, anilist_id):
        item_information = self.get_show(anilist_id)
        import xbmcgui
        xbmcgui.Dialog().textviewer('dssds', str(item_information))

    @guard_against_none()
    def _get_single_show_meta(self, anilist_id):
        return self._get_single_meta(anilist_id, "shows")

    @guard_against_none()
    def _get_single_show_meta_mal(self, mal_id):
        return self._get_single_meta_mal(mal_id, "shows")

    @guard_against_none(list)
    def get_show(self, anilist_id):
        """
        ...
        :rtype: dict
        """
        return self.get_show_list([self._get_single_show_meta(anilist_id)])[0]

    @guard_against_none(list)
    def get_show_mal(self, mal_id):
        """
        Returns a single show record from the database with full meta
        :param trakt_id: Shows Trakt ID
        :type trakt_id: int
        ...
        :rtype: dict
        """
        return self._get_single_show_meta_mal(mal_id)
    ...
    def _update_shows(self, list_to_update):
        ...
        sql_statement = """WITH requested(anilist_id, last_updated) AS (VALUES {}) select r.anilist_id, anilist.value as 
        anilist_object, anilist.meta_hash as anilist_meta_hash, tmdb_id, tmdb.value as tmdb_object, tmdb.meta_hash as 
        ...
        THEN 'true' else 'false' END as NeedsUpdate FROM requested as r LEFT JOIN shows as s on r.anilist_id = 
        s.anilist_id LEFT JOIN shows_meta as anilist on anilist.id = s.anilist_id and anilist.type = 'anilist' LEFT JOIN 
        ...
        'fanart' LEFT JOIN shows_meta as omdb on omdb.id = s.imdb_id and omdb.type = 'omdb' """.format(
            ",".join(
                "({},'{}')".format(
                    i.get("anilist_id", i.get("anilist_id")), get(i, "dateadded")
                )
    ...
    def _update_alt_mill_format_shows(self, trakt_list, mill_episodes=False):
        ...
        self.insert_anilist_shows(trakt_list)
        self._update_shows(trakt_list)
        self._alt_mill_if_needed(trakt_list, None, mill_episodes)
    ...
    def _identify_seasons_to_update(self, list_to_update):
        ...
        trakt.value as trakt_object, trakt.meta_hash as trakt_meta_hash, sh.anilist_id as anilist_id, anilist.value 
        as anilist_object, anilist.meta_hash as anilist_meta_hash, sh.tmdb_id as tmdb_show_id, se.tmdb_id as 
        ...
        seasons_meta as tmdb on tmdb.id = se.tmdb_id and tmdb.type = 'tmdb' LEFT JOIN seasons_meta as anilist on 
        anilist.id = se.anilist_id and anilist.type = 'anilist' LEFT JOIN seasons_meta as tvdb on tvdb.id 
    ...
    def _identify_episodes_to_update(self, list_to_update):
        ...
        ep.anilist_id, anilist.value as anilist_object, anilist.meta_hash as anilist_meta_hash, ep.tmdb_id as tmdb_id, 
        tmdb.value as tmdb_object, tmdb.meta_hash as tmdb_meta_hash, ep.tvdb_id as tvdb_id, tvdb.value as tvdb_object, 
        tvdb.meta_hash as tvdb_meta_hash, fanart.value as fanart_object, fanart.meta_hash as fanart_meta_hash, ep.imdb_id, 
        omdb.value as omdb_object, omdb.meta_hash as omdb_meta_hash, sh.tmdb_id as tmdb_show_id, sh.tvdb_id as tvdb_show_id, 
        sh.info as show_info, sh.art as show_art, sh.cast as show_cast, ep.trakt_season_id, se.tmdb_id as tmdb_season_id, 
        sh.tvdb_id as tvdb_season_id, se.info as season_info, se.art as season_art, se.cast as season_cast, CASE WHEN 
        ep.last_updated is null or (Datetime(ep.last_updated) < Datetime(r.last_updated)) THEN 'true' else 'false' END as 
        NeedsUpdate FROM requested as r LEFT JOIN episodes as ep on r.trakt_id = ep.trakt_id LEFT JOIN shows as sh on 
        sh.trakt_id = ep.trakt_show_id LEFT JOIN seasons as se on se.trakt_id = ep.trakt_season_id LEFT JOIN episodes_meta 
        as trakt on trakt.id = ep.trakt_id and trakt.type = 'trakt' LEFT JOIN episodes_meta as anilist on 
        anilist.id = ep.anilist_id and anilist.type = 'anilist' LEFT JOIN episodes_meta as tmdb on tmdb.id = ep.tmdb_id and 
        tmdb.type = 'tmdb' LEFT JOIN episodes_meta as tvdb on tvdb.id = ep.tvdb_id and tvdb.type = 'tvdb' LEFT JOIN 
        episodes_meta as fanart on fanart.id = ep.tvdb_id and fanart.type = 'fanart' LEFT JOIN episodes_meta as omdb on 
        omdb.id = ep.imdb_id and omdb.type = 'omdb' """.format(
            ",".join(
                "({},'{}')".format(i.get("trakt_id"), get(i, "dateadded"))
                for i in list_to_update
            )
    ...
    def format_tmdb_episodes(self, list_to_update, anilist_id, tmdb_id=None, trakt_id=None):
        ...
        # if formatted_items is None:
        #     return
        to_insert = self._filter_tmdb_items_that_needs_updating(list_to_update, "episodes")
    ...
    def format_simkl_episodes(self, list_to_update, anilist_id, tmdb_id=None, trakt_id=None):
        ...
        # if formatted_items is None:
        #     return
        to_insert = self._filter_simkl_items_that_needs_updating(list_to_update, "episodes")
    ...
    def _try_update_seasons(self, anilist_id, trakt_show_id, trakt_season_id=None):
        ...
        query = """SELECT s.trakt_id, value as trakt_object, s.anilist_id, s.trakt_show_id, sh.tmdb_id as tmdb_show_id, sh.tvdb_id 
    ...
    def _try_alt_update_episodes(self, anilist_id, trakt_show_id, trakt_season_id=None, trakt_id=None):
        show_meta = self._get_single_show_meta(anilist_id)
        self._update_alt_mill_format_shows(show_meta, True)
        query = """SELECT value as trakt_object, e.anilist_id, e.trakt_id, e.trakt_show_id, sh.tmdb_id as tmdb_show_id, sh.tvdb_id 
        as tvdb_show_id, sh.simkl_id as simkl_show_id FROM episodes as e INNER JOIN shows as sh on e.trakt_show_id = sh.trakt_id 
        INNER JOIN episodes_meta as m on m.id = e.trakt_id and m.type='trakt' where """
    ...
    def _try_update_mixed_episodes(self, trakt_items):
        self.insert_anilist_shows([i["show"] for i in trakt_items if i.get("show")])

        for i in trakt_items:
            if not i.get("show"):
                self.task_queue.put(self._get_single_show_meta, i["anilist_id"])
        self.task_queue.wait_completion()

        shows = self.fetchall(
            """SELECT value as anilist_object, s.anilist_id, s.trakt_id, s.tvdb_id, s.tmdb_id FROM shows as s 
        INNER JOIN shows_meta as m on m.id = s.anilist_id and  m.type='anilist' where s.anilist_id in ({})""".format(
                ",".join(g.UNICODE(i.get("anilist_id")) for i in trakt_items)
            )
        )

        for i in trakt_items:
            from resources.lib.indexers import simkl, tmdb2

            item_information = self.get_show(i["anilist_id"])
            ...
            if episode_indexer == "trakt":
                season = self.get_season_list(i["anilist_id"], trakt_id, no_paging=True)
            elif episode_indexer == "tmdb_season":
                tmdb_result = tmdb2.TMDBAPI().get_season(i["anilist_id"], item_information)
                self.format_tmdb_episodes(tmdb_result, i["anilist_id"], tmdb_id, trakt_id)
                ...
                if simkl_result:
                    self.format_simkl_episodes(simkl_result, i["anilist_id"], tmdb_id, trakt_id)

            # if trakt_id and trakt_id != 0:
            #     season = self.get_season_list(i["anilist_id"], trakt_id, no_paging=True)

            #     if i["anilist_id"] in {113538, 119661, 104578, 116742}:
            #         tmdb_result = tmdb2.TMDBAPI().get_season(i["anilist_id"], item_information)
            #         self.format_tmdb_episodes(tmdb_result, i["anilist_id"], tmdb_id, trakt_id)
            #     elif season:
            #         self._try_update_episodes(i["anilist_id"], trakt_id)
        ...
        # e.anilist_id, e.trakt_id, e.trakt_show_id, sh.tmdb_id as tmdb_show_id, sh.tvdb_id as tvdb_show_id FROM episodes as e INNER 
        # JOIN shows as sh on e.anilist_id = sh.anilist_id INNER JOIN episodes_meta as em on em.id = e.trakt_id and 
    ...
    def get_nextup_episodes(self, sort_by_last_watched=False):
        ...
        :rtype: list
        """
        query = """SELECT e.trakt_id, e.number AS episode_x, e.season AS season_x, e.anilist_id, e.trakt_show_id, em.value AS 
        episode, sm.value AS show, s.tmdb_id AS tmdb_show_id, s.tvdb_id AS tvdb_show_id, inner.last_watched_at, 
        e.air_date FROM episodes AS e INNER JOIN shows AS s ON s.anilist_id = e.anilist_id INNER JOIN (SELECT 
        ...
        nw.trakt_show_id AND e.season == nw.season AND e.number >= nw.number) WHERE e.season > 0 AND watched = 0
        ...
        ON e.trakt_id = em.id AND em.TYPE = 'trakt' LEFT JOIN shows_meta AS sm ON e.anilist_id = sm.id AND sm.TYPE 
        = 'anilist' """
