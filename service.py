...
from resources.lib.modules.kaito_version import do_version_change
from resources.lib.modules.kaitoMonitor import KaitoMonitor
...
monitor = KaitoMonitor()
try:
    xbmc.executebuiltin(
        'RunPlugin("plugin://plugin.video.kaito/?action=longLifeServiceManager")'
    )
    ...
    xbmc.executebuiltin(
        'RunPlugin("plugin://plugin.video.kaito/?action=torrentCacheCleanup")'
    )

    g.wait_for_abort(30)  # Sleep for a half a minute to allow widget loads to complete.
    while not monitor.abortRequested():
        xbmc.executebuiltin(
            'RunPlugin("plugin://plugin.video.kaito/?action=runMaintenance")'
        )
        if not g.wait_for_abort(15):  # Sleep to make sure tokens refreshed during maintenance
            xbmc.executebuiltin(
                'RunPlugin("plugin://plugin.video.kaito/?action=syncTraktActivities")'
            )
        if not g.wait_for_abort(15):  # Sleep to make sure we don't possibly clobber settings
            xbmc.executebuiltin(
                'RunPlugin("plugin://plugin.video.kaito/?action=cleanOrphanedMetadata")'
            )
        if not g.wait_for_abort(15):  # Sleep to make sure we don't possibly clobber settings
            xbmc.executebuiltin(
                'RunPlugin("plugin://plugin.video.kaito/?action=updateLocalTimezone")'
            )
